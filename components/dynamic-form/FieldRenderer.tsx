            ...
            return (
                <Field name={field.key}>
                    {({ field: formikField, form }: any) => {
                        ...
                        const dynamicPlaceholder = field.getPlaceholder ? field.getPlaceholder(dependentValues) : field.placeholder;
            ...
            return (
                <div className='space-y-1'>
                    ...
                    {field.description && field.description.trim() !== "" && (
                        <p className='text-[#7D7D7D] text-sm mt-1'>{field.description}</p>
                    )}
                    <ErrorMessage
                        name={field.key}
                        component='div'
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        form.setFieldValue(field.key, numericValue);
                                    }}
                                />
                                {dynamicDescription && dynamicDescription.trim() !== "" && (
                                    <p className='text-[#7D7D7D] text-sm mt-1'>{dynamicDescription}</p>
            ...
            return (
                <Field name={field.key}>
                    {({ field: formikField, form }: any) => {
                        ...
                        const placeholder = dynamicPlaceholder || (isMulti ? "Select options..." : "Select an option...");
                        ...
                        if (isMulti) {
                            ...
                            selectedOptions = currentValue ?
                                selectOptions.find(option => option.value === currentValue) || null :
                                null;
                        }
                        ...
                        const handleChange = (selectedOption: any) => {
                            if (isMulti) {
                                ...
                                dispatch(
                                    updateField({ key: field.key, value: values })
                                );
                            } else {
                                // For single select
            ...
            return (
                <Field name={field.key}>
                    {({ field: _, form }: any) => {
                        ...
                        const minDateValidator = field.validators?.find(v => v.type === "minDate");
                        const maxDateValidator = field.validators?.find(v => v.type === "maxDate");

                        const minDate = minDateValidator?.value ? new Date(minDateValidator.value as string) : undefined;
                        const maxDate = maxDateValidator?.value ? new Date(maxDateValidator.value as string) : undefined;
                ...
                <div>
                    ...
                    <Field name={field.key}>
                        {({ field: _, form }: any) => {
                            ...
                            const maxFiles = field.maxFiles || (isMultiple ? 10 : 1);
                            ...
                            const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
                                ...
                                if (oversizedFiles.length > 0) {
                                    ...
                                    alert(`File(s) too large. Maximum size is ${maxSizeMB}MB`);
                                    return;
                                }
                                ...
                                if (invalidFiles.length > 0) {
                                    alert(`Invalid file type(s). Allowed: ${allowedTypes.join(', ')}`);
                                    return;
                                }
                                ...
                                const selectedFiles = isMultiple ? fileMetadata : fileMetadata[0];
                            ...
                            const removeFile = (index: number) => {
                                if (isMultiple) {
                                    const fileToRemove = (currentFiles as FileMetadata[])[index];
                                    if (fileToRemove && fileToRemove.id) {
                                        sessionStorage.removeItem(`file_${fileToRemove.id}`);
                                    }
                                    const updatedFiles = Array.from(currentFiles as FileMetadata[]).filter((_, i) => i !== index);
                                    ...
                                    if (currentFiles && (currentFiles as FileMetadata).id) {
                                        sessionStorage.removeItem(`file_${(currentFiles as FileMetadata).id}`);
                                    }
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <div className="text-xs text-gray-500 mt-1">
                                            ...
                                            <div>Files: {minFiles} - {maxFiles}</div>
                                    ...
                                    {currentFiles && (
                                        <div className="space-y-2">
                                            {isMultiple ? (
                                                Array.from(currentFiles as FileMetadata[]).map((file: FileMetadata, index: number) => (
                                                ...
                                                currentFiles && (
                                                    <div className="flex items-center justify-between p-2 bg-gray-50 rounded border">
                                                        <div className="flex-1 min-w-0">
                                                            <div className="text-sm font-medium text-gray-900 truncate">
                                                                {(currentFiles as FileMetadata).name}
                                                            </div>
                                                            <div className="text-xs text-gray-500">
                                                                {formatFileSize((currentFiles as FileMetadata).size)} • {(currentFiles as FileMetadata).type}
                                                            </div>
                                                        </div>
                                                        <Button
                                                            type="button"
                ...
                <Field name={field.key}>
                    {({ field: formikField, form }: any) => {
                        try {
                            ...
                            if (!effectiveApiEndpoint || !effectiveValueKey || !effectiveLabelKey) {
                                return (
                                    <div className="text-red-500 text-sm p-4 border border-red-200 rounded bg-red-50">
                                        <strong>Configuration Error:</strong> Lookup field "{field.label}" is missing required configuration.
                                        Please provide apiEndpoint, valueKey, and labelKey in lookupConfig.
                                    </div>
                                );
                            }

                            // Set defaults only for properties that need them for logic to work
                            const minSearchLengthForLogic = minSearchLength ?? 0;

                            // Get dependent field values if callback is provided (supports multiple dependencies)
                            const dependentValues = React.useMemo(() =>
                                field.getDependentValue ? field.getDependentValue(form.values) : null,
                                [field.getDependentValue, form.values]
                            );

                            // Dynamic field properties based on dependent values
                            const dynamicDescription = React.useMemo(() =>
                                field.getDescription ? field.getDescription(dependentValues) : field.description,
                                [field.getDescription, dependentValues, field.description]
                            );
                            const dynamicPlaceholder = React.useMemo(() =>
                                field.getPlaceholder ? field.getPlaceholder(dependentValues) : field.placeholder,
                                [field.getPlaceholder, dependentValues, field.placeholder]
                            );
                            const isFieldDisabled = React.useMemo(() =>
                                field.isDisabled ? field.isDisabled(dependentValues) : field.disabled,
                                [field.isDisabled, dependentValues, field.disabled]
                            );
                            const isFieldHidden = React.useMemo(() =>
                                field.isHide ? field.isHide(dependentValues) : false,
                                [field.isHide, dependentValues]
                            );
                            const isFieldRequired = React.useMemo(() =>
                                field.isRequired ? field.isRequired(dependentValues) : field.required,
                                [field.isRequired, dependentValues, field.required]
                            );

                            // Handle default value logic
                            const getDefaultValue = () => {
                                if (typeof defaultValue === 'function') {
                                    return defaultValue(dependentValues, form.values);
                                }
                                return defaultValue;
                            };
                            ...
                            React.useEffect(() => {
                                if (!form.values[field.key] && defaultValue !== undefined) {
                                    const defaultVal = getDefaultValue();
                                    if (defaultVal !== undefined) {
                                        const isMultiple = field.multiple !== undefined ? field.multiple : false;

                                        if (isMultiple) {
                                            // Handle multiple selection defaults
                                            if (Array.isArray(defaultVal)) {
                                                form.setFieldValue(field.key, defaultVal);
                                                dispatch(updateField({ key: field.key, value: defaultVal }));
                                            } else {
                                                // Convert single default to array for multiple selection
                                                const defaultArray = defaultVal ? [defaultVal] : [];
                                                form.setFieldValue(field.key, defaultArray);
                                                dispatch(updateField({ key: field.key, value: defaultArray }));
                                            }
                                        } else {
                            ...
                            }, [defaultValue, dependentValues, form.values, field.key, field.multiple]);
                            ...
                            React.useEffect(() => {
                                ...
                                const shouldClear = dependentValues && Object.values(dependentValues).some(value =>
                                    value !== form.values[field.key] && form.values[field.key]
                                );
                                if (shouldClear) {
                                    const isMultiple = field.multiple !== undefined ? field.multiple : false;
                                    ...
                                    if (defaultValue !== undefined) {
                                        ...
                                        dispatch(updateField({ key: field.key, value: clearValue }));
                                    }
                                }
                            }, [dependentValues, defaultValue, form.values, field.key, isFieldHidden, field.multiple]);

                            // Cache for storing API results
                            const cacheKey = React.useMemo(() =>
                                `${effectiveApiEndpoint}_${inputValue}_${JSON.stringify(dependentValues) || 'none'}`,
                                [effectiveApiEndpoint, inputValue, dependentValues]
                            );
                            ...
                            const debouncedSearch = React.useCallback(
                                (searchTerm: string) => {
                                    ...
                                    timeoutRef.current = setTimeout(async () => {
                                        if (searchTerm.length < minSearchLengthForLogic) {
                                            setOptions([]);
                                            return;
                                        }
                                        ...
                                        try {
                                            ...
                                            if (effectiveCacheResults && cache.has(cacheKey)) {
                                            ...
                                            if (transformRequest && dependentValues) {
                                                requestParams = transformRequest(requestParams, dependentValues);
                                            }

                                            // Make API request
                                            const response = await fetch(`/api/${effectiveApiEndpoint}`, {
                                                method: effectiveMethod || 'GET',
                                                headers: {
                                                    'Content-Type': 'application/json',
                                                },
                                                body: effectiveMethod === 'POST' ? JSON.stringify(requestParams) : undefined,
                                            });
                                ...
                                [effectiveApiEndpoint, effectiveValueKey, effectiveLabelKey, effectiveSearchKey, effectiveDebounceMs, minSearchLengthForLogic, effectiveCacheResults, cacheKey, transformRequest, dependentValues, transformResponse, isFieldHidden]
                            );
                            ...
                            React.useEffect(() => {
                                ...
                                const loadInitialOptions = async () => {
                                    try {
                                        ...
                                        console.error('Initial options loading error:', error);
                                ...
                                loadInitialOptions();
                            }, [effectiveApiEndpoint, effectiveValueKey, effectiveLabelKey, effectiveSearchKey, effectiveDebounceMs, effectiveCacheResults, cacheKey, transformRequest, dependentValues, transformResponse, isFieldHidden]);
                                ...
                                try {
                                    ...
                                    if (selectedOption) {
                                        if (isMultiple) {
                                            ...
                                            const selectedValues = Array.isArray(selectedOption) ? selectedOption : [selectedOption];
                                            console.log('Setting multiple field values:', field.key, selectedValues);
                                            form.setFieldValue(field.key, selectedValues);
                                            dispatch(
                                                updateField({ key: field.key, value: selectedValues })
                                            );
                                        } else {
                                            // Handle clearing the selection
                                            console.log('Clearing field value:', field.key);
                                            const clearValue = isMultiple ? [] : null;
                                            form.setFieldValue(field.key, clearValue);
                                            dispatch(
                                                updateField({ key: field.key, value: clearValue })
                                            );
                                        }
                                    } else {
                                        // Handle clearing the selection
                                        ...
                                        form.setFieldValue(field.key, null);
                                        dispatch(
                                            updateField({ key: field.key, value: null })
                                        );
                                    }
                                } catch (error) {
                                    ...
                                    try {
                                        const isMultiple = field.multiple !== undefined ? field.multiple : false;
                                        if (isMultiple && Array.isArray(selectedOption)) {
                                            const simpleValues = selectedOption.map(option => 
                                                option?.value || option?.id || option
                                            );
                                            form.setFieldValue(field.key, simpleValues);
                                            dispatch(
                                                updateField({ key: field.key, value: simpleValues })
                                            );
                                        } else {
                                            const simpleValue = selectedOption?.value || selectedOption?.id || selectedOption;
                                            form.setFieldValue(field.key, simpleValue);
                                            dispatch(
                                                updateField({ key: field.key, value: simpleValue })
                                            );
                                        }
                                    } catch (fallbackError) {
                            ...
                            try {
                                if (isMultiple) {
                                    ...
                                    if (Array.isArray(currentValue)) {
                                        selectedOption = currentValue.map(value => 
                                            typeof value === 'object' ? value : safeOptions.find(option => option.value === value)
                                        ).filter(Boolean);
                                    } else if (currentValue) {
                                        // Convert single value to array for multiple selection
                                        const value = typeof currentValue === 'object' ? currentValue : safeOptions.find(option => option.value === currentValue);
                                    ...
                                    selectedOption = currentValue ? 
                                        (typeof currentValue === 'object' ? currentValue : safeOptions.find(option => option.value === currentValue)) : 
                                        null;
                                }
                            } catch (error) {
                                ...
                                <div>
                                    ...
                                    <div className="space-y-2">
                                        {(() => {
                                            try {
                                                // Get dynamic properties from field configuration
                                                const isSearchable = field.searchable !== undefined ? field.searchable : true;
                                                const isClearable = field.clearable !== undefined ? field.clearable : true;
                                                const isMultiple = field.multiple !== undefined ? field.multiple : false;
                                                
                                                // Debug logging for dynamic properties
                                                console.log(`Field ${field.key} properties:`, {
                                                ...
                                                return (
                                                    <ReactSelect
                                                        ...
                                                        isClearable={isClearable}
                                                        isMulti={isMultiple}
                                                        placeholder={dynamicPlaceholder || `Search ${field.label.toLowerCase()}...`}
                                                        styles={customStyles}
                                                        className="react-select-container"
                                                        classNamePrefix="react-select"
                                                        noOptionsMessage={() =>
                                                            isLoading ? "Loading..." :
                                                                safeInputValue.length < minSearchLengthForLogic ?
                                                        ...
                                                        formatOptionLabel={(option: any, { context }: any) => {
                                                            try {
                                                                ...
                                                                console.error('Error in formatOptionLabel:', error);
                                                                return 'Unknown';
                                                            }
                                                        }}
                                                    />
                                                ...
                                                console.error('Error rendering ReactSelect:', error);
                                                return (
                                                    <div className="p-4 border border-red-200 rounded bg-red-50 text-red-600">
                                                        Error rendering dropdown. Please refresh the page.
                                                    </div>
                                                );
                                            }
                                        })()}
                            ...
                            return (
                                <div className="text-red-500 text-sm p-4 border border-red-200 rounded bg-red-50">
                                    <strong>Error:</strong> Failed to render lookup field "{field.label}".
