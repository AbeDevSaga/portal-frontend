...
const body = {
  request: {
    ...
    data: {
      ...
      husbandBirthRegistrationUniqueIdentificationNumber: "BIRTH-0000123987",
    },
  },
};
...
export default function MarriageDetail() {
  ...
  const handleRenderApplicationDecisionButtons = (status: string) => {
    if (status === "SUBMITTED")
      return (
        <div className="flex flex-wrap items-center gap-3">
          <div className="px-5 text-center text-white py-2 rounded-sm bg-[#adada8]">
            SUBMITTED
          </div>
          <Button
            className="bg-[#cf1919] hover:bg-[#5c0b0b] transition-colors duration-300"
    ...
    if (status === "UNDER_REVIEW")
      return (
        <div className="px-5 text-center text-white py-2 rounded-sm bg-[#c4c924]">
          UNDER REVIEW
        </div>
      );
    if (status === "APPROVED")
      return (
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button
            className="bg-[#cc1d5d] hover:bg-[#a8174b] transition-colors duration-300"
            onClick={() => setShowTimer(true)}
            disabled={resolutionIsLoading}
          >
            View Certificate
  ...
  const supportingDocsArray = data?.data?.supporting_doc_url
    ? handleConvertSupportingDocStringToArray(data?.data?.supporting_doc_url)
    : [];
              ...
              <div className="flex flex-col justify-between w-full shadow-sm">
                ...
                <div className="space-y-2">
                  ...
                  supportingDocsArray.length > 0
                    ? supportingDocsArray.map((item, index) => (
                        <div
                          key={index}
                          onClick={() => {
                            setDisplayDoc(item.fileUrl);
            ...
            <div className="space-y-3">
              {requirementsandaction.map((requirementsandactionItem) => (
                ...
                >
                  ...
                  <Button
                    onClick={requirementsandactionItem.action}
                    className="bg-[#073954]"
                  >
