            ...
            {({ field: formikField, form }: any) => {
              ...
              const currentDependencyString = getDependencyKeys
                .map((key) => {
                  ...
                  return `${key}:${
                    value?.id || value?.value || JSON.stringify(value) || ""
                  }`;
                })
                .join("|");
              ...
              React.useEffect(() => {
                ...
                if (
                  ...
                  if (
                    ...
                    dispatch(
                      updateField({ key: field.key, value: newDefaultValue })
                    );
                  }
            ...
            return (
              <div className="space-y-2 flex flex-col">
                    ...
                    <Calendar
                      ...
                      disabled={(date) => {
                        ...
                        if (field.key === "dateOfMarriage" && formValues) {
                          ...
                          const shouldDisableFutureDates =
                            typeName !== "National";
                ...
                {description && description.trim() !== "" && (
                  <p className="text-[#7D7D7D] text-sm mt-1">{description}</p>
                )}
              </div>
            );
          },
              ...
              try {
                ...
                React.useEffect(() => {
                  try {
                    ...
                    if (!isFieldHidden) {
                      ...
                      console.log(
                        `[DEBUG] FileUpload ${field.key} - Form state changed:`,
                        {
                          fieldKey: field.key,
                    ...
                    console.error(
                      `[ERROR] FileUpload ${field.key} - Debug logging failed:`,
                      error
                    );
                  }
                }, [
                ...
                const handleFileChange = (
                  ...
                  try {
                    ...
                    console.error(
                      `[ERROR] FileUpload ${field.key} - File change handling failed:`,
                      error
                    );
                    alert(
                ...
                const removeFile = (index: number) => {
                  try {
                    if (isMultiple) {
                      ...
                      const updatedFiles = currentFilesArray.filter(
                        (_, i) => i !== index
                      );
                    ...
                    console.error(
                      `[ERROR] FileUpload ${field.key} - File removal failed:`,
                      error
                    );
                    alert(
                      `Error removing file: ${
                ...
                const formatFileSize = (bytes: number) => {
                  try {
                    ...
                    console.error(
                      `[ERROR] FileUpload ${field.key} - File size formatting failed:`,
                      error
                    );
                    return "Unknown size";
                  }
                ...
                console.error(
                  `[ERROR] FileUpload ${field.key} - Rendering failed:`,
                  error
                );
                return (
                  <div className="p-4 border border-red-200 rounded bg-red-50 text-red-600">
                    <strong>Error:</strong> Failed to render file upload field "
                    {field.label}".
                    <br />
                    <small>
                      Error details:{" "}
          ...
          <Field name={field.key}>
            {({ field: formikField, form }: any) => {
              try {
                ...
                // Get dependent field values if callback is provided
                let formdependentValues = field.getDependentValue
                  ? field.getDependentValue(form.values)
                  : null;
                // Normalize dependent values (if object, take first value)
                if (
                  formdependentValues &&
                  typeof formdependentValues === "object"
                ) {
                  const values = Object.values(formdependentValues);
                  formdependentValues = values.length > 0 ? values[0] : null;
                }
                const formdependentKey = field.getDependentKey
                  ? field.getDependentKey(form.values)
                  : null;
                console.log("form dependent key: ", formdependentKey);
                console.log("form dependent value: ", formdependentValues);
                ...
                if (
                  ...
                  return (
                    <div className="text-red-500 text-sm p-4 border border-red-200 rounded bg-red-50">
                      <strong>Configuration Error:</strong> Lookup field "
                ...
                React.useEffect(() => {
                  if (!form.values[field.key] && defaultValue !== undefined) {
                    ...
                    if (defaultVal !== undefined) {
                      const isMultiple =
                        field.multiple !== undefined ? field.multiple : false;
                ...
                React.useEffect(() => {
                  ...
                  const currentDependentValues =
                    field.getDependentValue?.(form.values) || null;
                  ...
                  (searchTerm: string) => {
                    ...
                    timeoutRef.current = setTimeout(async () => {
                      ...
                      if (
                        ...
                        searchTerm.length < minSearchLengthForLogic
                      ) {
                        setOptions([]);
                        return;
                      }
                      ...
                      try {
                        ...
                        if (formdependentKey && formdependentValues) {
                          requestParams[formdependentKey] = formdependentValues;
                        } else {
                          requestParams[formdependentKey || "search"] =
                            searchTerm;
                        }

                        // Add dependent values to request if transformRequest is provide
                  ...
                  try {
                    ...
                    if (selectedOption) {
                      if (isMultiple) {
                        ...
                        form.setFieldValue(field.key, selectedOption);
                        dispatch(
                          updateField({
                            key: field.key,
                    ...
                    try {
                      ...
                      if (isMultiple && Array.isArray(selectedOption)) {
                        const simpleValues = selectedOption.map(
                          (option) => option?.value || option?.id || option
                        );
                ...
                return (
                  <div className="text-red-500 text-sm p-4 border border-red-200 rounded bg-red-50">
                    <strong>Error:</strong> Failed to render lookup field "
                    {field.label}". Please refresh the page or contact support.
                    <br />
                    <small>
                      Error details:{" "}
                      {error instanceof Error ? error.message : "Unknown error"}
            ...
            {({ field: formikField, form }: any) => {
              ...
              React.useEffect(() => {
                if (form.values[field.key]) {
                  ...
                  if (
                    typeof form.values[field.key] === "string" &&
        ...
        return (
          <Field name={field.key}>
            {({ field: formikField, form }: any) => {
              ...
              // Special handling for hospital notifications endpoint
              const isHospitalNotificationEndpoint =
                apiEndpoint.includes("hosital-notifications") ||
                apiEndpoint.includes("hospital-notifications");
              ...
              const debouncedSearch = React.useCallback(
                (searchTerm: string) => {
                  ...
                  timeoutRef.current = setTimeout(async () => {
                    ...
                    try {
                      ...
                      // For hospital notifications, we want the format:
                      // https://crvs-birth.itsidx.com/api/v1/hosital-notifications/{searchTerm}
                      if (isHospitalNotificationEndpoint) {
                        // Remove any trailing slashes
                        modifiedEndpoint = modifiedEndpoint.replace(/\/$/, "");
                        // Append the search term directly to the endpoint path
                        modifiedEndpoint = `${modifiedEndpoint}/${encodeURIComponent(
                          searchTerm
                        )}`;
                      } else {
                        // Original logic for other endpoints
                        if (searchFormat === "path") {
                          // Handle path format - insert search term into the endpoint
                          const searchPlaceholders = [
                            "{search}",
                            "{id}",
                            "{term}",
                          ...
                          for (const placeholder of searchPlaceholders) {
                            if (modifiedEndpoint.includes(placeholder)) {
                              modifiedEndpoint = modifiedEndpoint.replace(
                                placeholder,
                                encodeURIComponent(searchTerm)
                              );
                          ...
                          // If no placeholder found, append the search term
                          if (
                            !searchPlaceholders.some((p) =>
                              modifiedEndpoint.includes(p)
                            )
                          ) {
                            modifiedEndpoint = modifiedEndpoint.replace(
                              /\/$/,
                          ...
                          // Query format - add search term as query parameter
                          requestParams[searchKey] = searchTerm;
                        }
                      ...
                      const response = await fetch(modifiedEndpoint, {
                        ...
                        body:
                          method === "POST" && !isHospitalNotificationEndpoint
                            ? JSON.stringify(requestParams)
                            : undefined,
                      });
                      ...
                      if (transformResponse) {
                        ...
                        // Default transformation
                        transformedOptions = Array.isArray(data)
                          ? data.map((item: any) => ({
                              value: item[valueKey],
                              label: item[labelKey],
                              ...item,
                ...
                [
                  ...
                  transformRequest,
                  transformResponse,
                  isHospitalNotificationEndpoint,
                  baseUrl,
                  isExternal,
                  method,
                  inputSearchConfig?.additionalParams,
